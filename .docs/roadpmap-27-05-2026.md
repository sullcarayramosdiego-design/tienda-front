...
* **⚠️ Brecha en Backend:**
  * **Sin integración con Órdenes:** El servicio de creación de pedidos (`OrdersService.create`) calcula costos fijos (`shipping = 15.0`) y descuento hardcodeado (`discount = 0`). **No valida si el usuario cuenta con una suscripción VIP** para otorgar envíos gratis o el 10% de descuento directo en catálogo.
  * **Sin Panel de Administración:** No existen rutas `/admin` para que un Administrador pueda crear/editar planes de suscripción, consultar la lista global de suscriptores activos, o forzar bajas/pausas de membresías de clientes.
...
* **⚠️ Brecha en Frontend:**
  * **Sin integración en Checkout:** En la página de pago (`/checkout`), no se refleja el descuento premium del 10% ni el envío gratis si el usuario está suscrito al plan VIP.
  * **Sin Vista de Administración:** La barra lateral de administración (`AdminSidebar.tsx`) no contiene acceso a la gestión de suscripciones o planes, y la página del administrador carece de estas métricas.
...
* **⚠️ Brecha en Backend:**
  * **Redención Desconectada del Pedido:** La ruta `/loyalty/redeem` realiza una resta directa en el balance de puntos, pero **no hay manera de aplicar ese descuento en la transacción real de una orden (`OrdersService.create`)**. El DTO de pedidos no soporta montos de descuento por lealtad.
  * **Sin Operación de Administración:** Falta un servicio y endpoints admin para ajustar balances de puntos por reclamos, auditar transacciones a nivel administrativo o listar usuarios destacados en los tiers de lealtad.
...
* **⚠️ Brecha en Frontend:**
  * **Sin integración en Checkout:** No hay un selector en el checkout que permita al cliente decir: *"Deseo usar 500 puntos de mi balance para descontar S/. 5.00 de este pedido"*.
  * **Discrepancia de Reglas:** El checkout muestra una estimación estática de puntos a ganar (`floor(subtotal / 2)`), la cual choca con la regla oficial del backend (`floor(orderTotal * 10)`).
  * **Sin Panel Admin:** El administrador no puede ver los puntos acumulados por los usuarios en la sección `/admin/users` ni auditar transacciones.
...
* **⚠️ Brecha en Frontend (Crítica):**
  * **UI con Datos Hardcodeados:** La página `/account/referrals/page.tsx` es **100% estática**. Genera un código de referido aleatorio con `Math.random()`, simula un historial de invitados falso y no interactúa con ningún servicio de la API de referidos.
  * **Sin Flujo de Registro:** La página de creación de cuenta (`/register`) no tiene soporte para capturar el parámetro `?ref=xxx` de la URL para enviar la solicitud de `Post /referrals/apply` tras la creación exitosa del usuario.
  * **Sin Vista de Administración:** A pesar de que el backend cuenta con `/referrals/admin`, el frontend de administración no posee ninguna pantalla para visualizar este flujo ni auditar fraudes de autoreferidos.
