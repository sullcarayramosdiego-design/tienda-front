...
export function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      setError(message);
      toast({ title: "Error al Iniciar Sesión", description: message, type: "error" });
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setError(null);
      const result = await signIn("google", { callbackUrl: "/catalog", redirect: true });
      if (result?.error) {
        setError("Error al iniciar sesión con Google. Intenta de nuevo.");
        toast({ title: "Error", description: "No se pudo iniciar sesión con Google", type: "error" });
      }
    } catch (err) {
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormControl>
                      <div className="relative group">
                        ...
                        <Input
                          ...
                          className={cn(
                            "pl-10 h-11 transition-all duration-200 rounded-xl bg-background/50 border-primary/10 focus-visible:ring-2 focus-visible:ring-offset-0",
                    ...
                    <FormControl>
                      <div className="relative group">
                        ...
                        <Input
                          ...
                          disabled={form.formState.isSubmitting}
                          className={cn(
                            "pl-10 pr-10 h-11 transition-all duration-200 rounded-xl bg-background/50 border-primary/10 focus-visible:ring-2 focus-visible:ring-offset-0",
                        ...
                        <button
                          ...
                          disabled={form.formState.isSubmitting}
                          className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground focus:outline-none rounded-full p-1 transition-colors duration-200"
              ...
              <div className="flex items-center justify-between pt-1">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    type="checkbox"
                    disabled={form.formState.isSubmitting}
                    className="h-4 w-4 rounded border-primary/20 text-primary focus:ring-primary focus:ring-offset-background bg-background/50 accent-primary cursor-pointer disabled:opacity-50"
                  />
              ...
              >
                {form.formState.isSubmitting ? (
                  <><Loader2 className="h-4 w-4 animate-spin text-white" /><span>Iniciando sesión...</span></>
                ) : (
