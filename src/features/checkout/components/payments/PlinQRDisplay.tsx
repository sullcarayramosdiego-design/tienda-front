...
interface PlinQRDisplayProps {
  amount: number;
  onConfirm?: () => void;
}

export function PlinQRDisplay({ amount, onConfirm }: PlinQRDisplayProps) {
      ...
      <div className="flex flex-col items-center gap-4 py-10 animate-in fade-in zoom-in duration-500">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            {fmtPrice(amount)} procesado con <span className="font-bold text-foreground">Plin</span>
          </p>
        </div>
      </div>
    );
  ...
  if (status === 'verifying') {
    return (
      <div className="flex flex-col items-center gap-4 py-10">
        ...
        <p className="text-xs font-semibold text-muted-foreground">Verificando con Plin...</p>
      </div>
    );
  }

  const size = 180; const cols = Math.floor(size / 7);
  const qr = Array.from({ length: cols }, (_, r) =>
    Array.from({ length: cols }, (_, c) => {
      ...
      return ((r * 29 + c * 13 + Math.floor(amount * 100) + 5) % 7) < 3 ? 1 : 0;
    })
  );
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="flex items-center gap-2.5">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#00A8A9] to-[#00C9CA] flex items-center justify-center shadow-md">
          <span className="text-white font-black text-base">P</span>
        </div>
        <div>
          <p className="font-heading font-extrabold text-sm text-foreground">Paga con Plin</p>
          <p className="text-[10px] text-muted-foreground">Escanea el QR desde tu app bancaria</p>
        </div>
      </div>
      ...
      <div className="relative bg-white rounded-2xl p-4 shadow-lg border border-primary/10">
        <svg width={size} height={size}>
          {qr.map((row, ri) =>
            row.map((cell, ci) =>
              cell ? (
                <rect key={`${ri}-${ci}`} x={ci * 7} y={ri * 7} width={6} height={6} fill="#0f766e" rx={1} />
              ) : null
            )
          )}
        </svg>
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-[#00A8A9] to-[#00C9CA] flex items-center justify-center shadow-md border-2 border-white">
      ...
      <div className="w-full">
        <p className="text-[9px] font-black uppercase tracking-wider text-muted-foreground mb-2 text-center">
          Bancos participantes
        </p>
        <div className="flex flex-wrap justify-center gap-1.5">
          {['BBVA', 'Interbank', 'Scotiabank', 'BCP', 'BanBif'].map((b) => (
            <span key={b} className="text-[10px] bg-muted/50 border border-primary/10 rounded-lg px-2.5 py-1 text-muted-foreground font-bold">
              {b}
            </span>
          ))}
        </div>
      ...
      >
        ...
        Simular confirmación Plin (Demo)
      </button>
    </div>
  );
}
